...
| --- | --- | --- |
| `byLayer` | `Record<string, Record<string, Record<string, unknown>>>` | One entry per layer that defines at least one of the keys, each exactly what a read of that layer alone returns. |
| `layersSearched` | `string[]` | Every layer that was read, the ones defining none of the keys included. |
